          ...
          <TabsList className="container flex flex-col items-center justify-center gap-4 sm:flex-row md:gap-10">
            {tabs.map((tab) => (
              <TabsTrigger
                key={tab.value}
                value={tab.value}
                className="flex items-center gap-2 rounded-xl px-4 py-3 text-sm font-semibold text-gray-700 dark:text-gray-300 data-[state=active]:bg-muted data-[state=active]:text-gray-900 dark:data-[state=active]:text-gray-100"
              >
                {tab.icon} {tab.label}
              </TabsTrigger>
            ))}
